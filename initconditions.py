...
def initconds(ics, k, z_p, z_i, alpha, beta, Background, clustering, **cosmo):
    ...
    # initial conditions
    x_i, y_i, r_i, v_i, h_i, w_q_i, Omega_q_i, Omega_m_i = ics
    
    # primordial gravitational field phi
    A = 50./9. * np.pi**2 * delta_H**2 * A_s**2
    Phi_p =  - np.sqrt(A) * (k)**(-3./2.) * (c*k/H0)**((n_s - 1.)/2.) * Omega0_m
...
#    Phi_p =  - np.sqrt(A) * (k/ks)**((n_s - 1.)/2.)
    ...
    # coupling
    Q_m_i = np.sqrt(2./3.) * beta * x_i * Omega_m_i; Q_q_i = - Q_m_i
    ...
    #- Phi_i
    Phi_i = 9./10. * Phi_p * transfer_func(k, **cosmo)
    #- Delta_m_i
    Delta_m_i = 2./3. * (c * k/((1./(1. + z_i)) * h_i))**2 * Phi_i/Omega_m_i * (1. + mu_delta) # changing sign check()
    #- u_m_i
    u_m_i = 2./3. * 1./((1. + w_q_i * Omega_q_i) * h_i) * Phi_i
    #- Delta_q_i
    Delta_q_i = 2./3. * (c * k/((1./(1. + z_i)) * h_i))**2 * Phi_i/Omega_q_i * mu_delta
    #- u_q_i
    u_q_i = u_m_i
...
#    
#    ics = quintini(z_p, z_i, 0.0, 0.0, **cosmo)
#    Delta_m_i0 = np.array([initconds(ics, k[i], z_p, z_i, 0.0, 0.0, False, True, **cosmo)[5] for i in range(len(k))])
#
...
#    plt.show()
    ...
    #print initconds(k, z_p, z_i, alpha, beta, Background = True, **cosmo)
